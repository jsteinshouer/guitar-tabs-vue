...
component extends="coldbox.system.EventHandler"{
	...
	*/
	function aroundHandler( event, rc, prc, targetAction, eventArguments ){
		try{
			...
			prc.response = getModel( "api.Response" );
	...
	*/
	function onError( event, rc, prc, faultAction, exception, eventArguments ){
		...
		if( !structKeyExists( prc, "api.Response" ) ){ 
			prc.response = getModel( "api.Response" ); 
		}
	...
	*/
	private function parseParameters( 
		...
		if ( event.getHTTPHeader("Content-Type").find("application/json") ){
			var params = event.getHTTPContent( json=true );

			if (isJSON(params)) {
				params = deserializeJSON(params);
			}
			else if (!isStruct(params)) {
				params = deserializeJSON(ToString(params));
			}

			try {
				structAppend(rc, params);
			}
			catch(Any e) {
				log.error( "Invalid JSON Format!: #e.message# #e.detail#", { 
					"rc" = rc,
					"httpContent" = toString(getHTTPRequestData().content), 
